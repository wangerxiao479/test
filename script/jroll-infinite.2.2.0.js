...
(function(window, document, JRoll) {
    ...
    JRoll.prototype.infinite = function(params) {
        ...
        if (typeof options.getData === 'function') {
            if (options.targetId) {
                document.getElementById(options.targetId).insertAdjacentHTML('beforeend', options.loadingTip)
            } else {
        ...
        function errorCallback() {
            var div = me.scroller.querySelector('.jroll-infinite-tip') || document.createElement('div')
        ...
        function callback(data) {
            ...
            if (!data) return;
            if (data.length == 0 && options.isShowEmpty == false) {
                html = "<section class='jroll-infinite-page' style='display:none'>";
            } else {
            ...
            if (options.targetId) {
                var dom = document.getElementById(options.targetId)
                if (dom.lastElementChild) dom.removeChild(dom.lastElementChild)
